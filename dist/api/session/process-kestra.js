...
async function handleKestraWebhook(req, res) {
    try {
...
async function processCompletedExecution(sessionId, webhookData) {
    try {
        // Fetch session state
        const session = await sessionStore.getSession(sessionId);
        if (!session) {
        ...
        const geminiTask = taskRuns.find((task) => task.taskId === 'gemini_reflection');
        ...
        if (!geminiText) {
            ...
            if (apiResponse.ok) {
                ...
                const geminiTaskFromApi = executionData.taskRunList?.find((task) => task.taskId === 'gemini_reflection');
...
export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            ...
            const geminiTask = executionData.taskRunList?.find((task) => task.taskId === 'gemini_reflection');
            if (!geminiTask) {
